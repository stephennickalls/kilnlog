...
export function useFiringStats(selectedFiring, nowUnix) {
  ...
  const targetRate = computed(() => {
    ...
    if (elapsedMins >= schedule.at(-1).offset_minutes) return fmtRate(0)
    ...
    if (deltaMins === 0) return fmtRate(0)
  ...
  return { duration, readingCount, elapsed, rateOfChange, targetRate, targetTemp }
}